          ...
          <div class="flex items-center space-x-3 mb-6">
            ...
            <div>
              ...
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                <span v-if="fetchingUser" class="animate-pulse">Loading user info...</span>
                <span v-else>
...
const fetchUserInfo = async () => {
  try {
    ...
    if (isForbiddenAdmin.value) {
      error.value = 'No estás autorizado para cambiar la contraseña de otro administrador.'
    }
  } catch (err) {
...
const handleResetPassword = async () => {
  ...
  try {
    ...
    successMessage.value = t('users.passwordChangedSuccessfully') || 'Password updated successfully!'
    ...
    error.value = err.response?.data?.message || t('users.changePasswordError') || 'Failed to change password.'
  } finally {
    loading.value = false
  }
}
