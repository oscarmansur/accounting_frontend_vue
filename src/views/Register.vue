...
const handleRegister = async (e) => {
  ...
  try {
    ...
    successMessage.value = t('auth.registrationVerifyEmail') || 'Registration successful! Please check your email to verify your account.'
    ...
    <Transition name="fade" appear>
      <div class="max-w-md w-full bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-lg shadow-xl p-8 relative z-10">
        ...
        <div class="text-center mb-8">
          <h2 class="text-3xl font-bold text-gray-800 dark:text-gray-100">{{ $t('auth.createAccount') }}</h2>
          <p class="text-gray-600 dark:text-gray-400 mt-2">{{ $t('auth.signUpSubtitle') }}</p>
        </div>
        ...
        <form @submit.prevent="handleRegister" class="space-y-6">
          ...
          <div>
            <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-100">{{ $t('auth.name') }}</label>
            <input
              id="name"
              v-model="name"
              type="text"
          ...
          <div class="text-center mt-4">
            <p class="text-sm text-gray-600 dark:text-gray-400">
              ...
              <router-link :to="{ name: 'login' }" class="font-medium text-blue-600 hover:text-blue-500 dark:text-blue-400">
                {{ $t('auth.signIn') }}
              </router-link>
            </p>
          </div>
