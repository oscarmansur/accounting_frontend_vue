...
const handleLogin = async (e) => {
  ...
  try {
    ...
    // Trigger animation
    authStore.setWelcomeAnimation(true)
    
    // Wait for curtain to cover (approx 800ms-1s)
    setTimeout(async () => {
      await router.push({ name: 'dashboard' })
      
      // Wait a moment before revealing
      setTimeout(() => {
        authStore.setWelcomeAnimation(false)
      }, 500)
    }, 1200)

  } catch (err) {
    console.error('Login form error:', err)
    error.value = err.response?.data?.detail || t('auth.invalidCredentials')
    loading.value = false // Only stop loading on error, success keeps loading until navigation
  }
}
</script>
    ...
    <Transition name="fade" appear>
      <div class="max-w-md w-full bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-lg shadow-xl p-8 relative z-10">
        ...
        <div class="text-center mb-8">
          <h2 class="text-3xl font-bold text-gray-800 dark:text-gray-100">{{ $t('auth.welcomeBack') }}</h2>
        ...
        <form @submit.prevent="handleLogin" class="space-y-6">
          ...
          <div>
            <label for="password" class="block text-sm font-medium text-gray-700 dark:text-gray-100">{{ $t('auth.password') }}</label>
            <div class="relative">
              <input
                id="password"
                v-model="password"
            ...
            <div class="flex justify-end mt-1">
              <router-link :to="{ name: 'forgot-password' }" class="text-sm font-medium text-blue-600 hover:text-blue-500 dark:text-blue-400">
        ...
        <div class="text-center mt-4">
          <p class="text-sm text-gray-600 dark:text-gray-400">
            {{ $t('auth.noAccount') }}
            <router-link :to="{ name: 'register' }" class="font-medium text-blue-600 hover:text-blue-500 dark:text-blue-400">
              {{ $t('auth.signUp') }}
            </router-link>
          </p>
        </div>
